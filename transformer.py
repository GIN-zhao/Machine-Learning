...
class AttentionHead(nn.Module):
    ...
    def forward(self,query,key,value,mask=None,dropout=None):
        ...
        scores = torch.bmm(query,key.tranpose(1,2))/sqrt(query.size(-1))
        if mask is not None:
...
class MultiHeadAttention(nn.Module):
    def __init__(self, h,d_model,dropout=0.1):
        ...
        self.heads = [AttentionHead(embed_dim=embed_dim,head_dim=head_dim) for i in range(num_heads)]
    ...
    def forward(self,query,key,value,mask=None):
        ...
        for head in self.heads:
            x_,attn_ = head(query,key,value,mask,self.dropout)
            x.append(x_)
            self.attn.append(attn_)
        x = torch.cat(x,dim=1)
        self.attn = torch.cat(self.attn,dim=1)
